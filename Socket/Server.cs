...
namespace ChatRoom
{
    internal class Server
    {
        ...
        public event Action<EndPoint, string>? MessageReceived;  // (clientEndpoint, message)
        public event Action<EndPoint>? ClientDisconnected;        // (clientEndpoint)
        public Server(string ip_str, string port_str, int maxnum,ConnectionManager cm)
        {
            ...
            EndPoint endPoint = socket.LocalEndPoint;// 获取服务器的IP地址和端口号作为标识
            this.cm = cm;
        ...
        public void Accept()
        {
            ...
            EndPoint endPoint = newclient.RemoteEndPoint; // 获取客户端的IP地址和端口号作为标识
        ...
        private void ReceiveLoop(Socket newclient, EndPoint endPoint)
        {
            ...
            try
            {
                while (true)
                { 
                    int len = newclient.Receive(buffer);

                    if (len > 0)
                    {
                        string message = System.Text.Encoding.UTF8.GetString(buffer, 0, len);
        ...
        public void Send(string message, EndPoint endPoint)
        {
            byte[] data = System.Text.Encoding.UTF8.GetBytes(message);
            Socket client = cm.ClientList[endPoint];// 从客户端列表中获取指定IP的客户端连接
            client.Send(data);
        }
